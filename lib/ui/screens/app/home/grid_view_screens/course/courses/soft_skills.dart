...
class SoftSkills extends StatelessWidget {
  ...
  Future<List<CoursesModel>> fetchSoftSkillsCourses() async {
    final allCourses = await CourseServices().fetchCourses();
    return allCourses
        .where((course) => course.type?.toLowerCase() == 'soft skills')
        .toList();
  }
    ...
    return FutureBuilder<List<CoursesModel>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return const Center(child: Text('No Soft Skills Courses available.'));
        } else {
